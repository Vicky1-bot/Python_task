...
def get_weather_data():
    ...
    # Query filter to match a single document with the given country and city
    query = {"location.country": country, "location.name": city}

    try:
        # Retrieve a single document matching the filter from the collection
        data = weather_collection.find_one(query)

        if data is None:
            logger.warning(f"No data found for {country} - {city}")
            return {"error": "No data found for the specified country and city"}, 404

        # Serialize data using bson.json_util
        json_data = json_util.dumps(data)
        logger.info(f"Successfully retrieved weather data for {country} - {city}")
        return json_data, 200
    except Exception as e:
        logger.error(f"Error occurred while retrieving weather data: {str(e)}")
